...
import { Image as ImageIcon, Music, Type, Plus, X, Trash2, Play, Pause, Smile } from 'lucide-react';
import { db, id, ImageAsset, MusicAsset, TextAsset, EmojiAsset } from '../lib/instantdb';
...
export default function MediaLibrary() {
  ...
  const tabs = [
    { id: 'images' as const, icon: ImageIcon, label: 'Zdjęcia', count: images.length },
    { id: 'music' as const, icon: Music, label: 'Muzyka', count: musicList.length },
    { id: 'texts' as const, icon: Type, label: 'Teksty', count: texts.length },
    { id: 'emojis' as const, icon: Smile, label: 'Emoji', count: emojis.length },
  ];
  ...
  return (
    <div className="h-full flex flex-col bg-black border-r border-gray-800">
      ...
      <div className="p-4 border-b border-gray-800">
        <h2 className="text-lg font-bold text-white">Biblioteka Zasobów</h2>
        <p className="text-xs text-white/50 mt-1">Zarządzaj zasobami do TikToków</p>
      </div>
      ...
      <div className="grid grid-cols-4 border-b border-gray-800">
        {tabs.map((tab) => (
          ...
          >
            ...
            <span>{tab.label}</span>
            <span className="text-[9px] text-white/40">({tab.count})</span>
          </button>
        ))}
      </div>
      ...
      <div className="flex-1 overflow-y-auto p-3 space-y-3">
        {isLoading ? (
          <div className="text-white/50 text-sm text-center py-8">Ładowanie...</div>
        ) : (
          <>
            {/* Images Tab */}
            {activeTab === 'images' && (
              <div className="space-y-3">
                ...
                >
                  <div className="flex flex-col items-center gap-1">
                    ...
                    <span className="text-sm font-medium">
                      {isDragging ? 'Upuść tutaj!' : 'Dodaj zdjęcia'}
                    </span>
                ...
                <div className="grid grid-cols-2 gap-2">
                  {images
                    .sort((a: ImageAsset, b: ImageAsset) => b.createdAt - a.createdAt)
            ...
            {activeTab === 'music' && (
              <div className="space-y-3">
                ...
                <div className="space-y-2">
                  {musicList
                    .sort((a: MusicAsset, b: MusicAsset) => b.createdAt - a.createdAt)
                    .map((music: MusicAsset) => {
                      ...
                      return (
                        ...
                        >
                          ...
                          <span className={`text-sm truncate flex-1 ${isPlaying ? 'text-white' : 'text-white/80'}`}>
                            {music.name}
                          </span>
                          <button
                            onClick={() => {
              ...
              <div className="space-y-3">
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    >
                      <Plus className="w-4 h-4" />
                      Dodaj {newText.includes('\n') ? 'teksty' : 'tekst'}
                    </button>
                  </div>
                  <p className="text-xs text-white/40 text-center">
                    Wklej wiele linii - każda stanie się osobnym tekstem
                  </p>
                </div>

                <div className="space-y-2">
                  {texts
                    .sort((a: TextAsset, b: TextAsset) => b.createdAt - a.createdAt)
                    .map((text: TextAsset) => (
                      <div
                        key={text.id}
                ...
                <div className="space-y-2">
                  <div className="flex gap-2">
                    <input
                      ...
                      placeholder="Wklej emoji..."
                      className="flex-1 p-3 bg-surface-light border border-gray-800 rounded-lg text-2xl text-center text-white placeholder-white/40 outline-none focus:border-white/30"
                    />
                    <button
                      onClick={handleAddEmoji}
                      disabled={!newEmoji.trim()}
                      className="px-4 bg-white text-black rounded-lg disabled:opacity-50 disabled:cursor-not-allowed hover:bg-white/90 transition-colors"
                    >
                      <Plus className="w-5 h-5" />
                    </button>
                  </div>
                  <p className="text-xs text-white/40 text-center">
                    Dodaj własne emoji do użycia w filmikach
                  </p>
                </div>

                <div className="grid grid-cols-4 gap-2">
                  {emojis
                    .sort((a: EmojiAsset, b: EmojiAsset) => b.createdAt - a.createdAt)
                    .map((emoji: EmojiAsset) => (
                      <div
                        key={emoji.id}
                        className="relative group aspect-square rounded-lg bg-surface border border-gray-800 flex items-center justify-center text-3xl"
                      >
                        {emoji.emoji}
                        <button
                          onClick={() => deleteEmoji(emoji.id)}
                          className="absolute top-0.5 right-0.5 p-1 bg-black/70 rounded-full opacity-0 group-hover:opacity-100 transition-opacity hover:bg-red-600"
                        >
